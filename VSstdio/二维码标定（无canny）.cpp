...
int main()
{
	namedWindow("框选"); namedWindow("s1");
	VideoCapture capture(0);
	while (1)
	{
		Start = getTickCount() * 1000 / getTickFrequency();


		capture >> src;
		//resize(frame, srcImage, Size(640, 460));
		...
		//resize(src, src, Size(360, 780));
		Mat src_all = src.clone();
		Mat yuantu = src.clone();
		//resize(src, src, Size(320, 230));
		//彩色图转灰度图
		...
		Mat so = Mat::zeros(src.size(), CV_8UC3);
		//指定阀值进行二值化
		threshold(src_gray, threshold_output, 140, 255, THRESH_BINARY);
		namedWindow("阈值图");
		...
		a = getTickCount() * 1000 / getTickFrequency();
		if (contours2.size() == 3 || contours2.size() == 4)
		{
			...
			for (int i = 0; i < 4; i++)
			{
				line(yuantu, fourPoint2f[i % 4], fourPoint2f[(i + 1) % 4]
					, Scalar(0, 0, 255), 3);
			}
			contours2.clear();
			/*namedWindow("框选");*/
		...
		waitKey(1);
		Stop = getTickCount() * 1000 / getTickFrequency();
		Tim = (Stop - Start);      // 使用了xxxm秒
		b = a - Start;
		cout << "全程" << Tim << endl;
		cout << "轮廓" << b << endl;
	}
